...
​	Notify 方法则会在释放锁的同时，唤醒对象 Wait Set 中等待的线程，顺序是不确定的，之后，正在wait的方法会获得锁，继续执行。notify后并不会阻塞，而是会继续执行。
...
public class LazySingleton {
    private LazySingleton() {}   //构造函数私有
    private static volatile LazySingleton instance = null;    //注意三个限定符的顺序，尤其是volatile
    public  static synchronized LazySingleton getInstance() { //注意synchronized的位置
        if (instance == null)
            instance = new LazySingleton();
        return instance;
    }
}
/* volatile就能够防止这条指令发生指令重排：instance = new LazySingleton();这一行代码可以分解为3行伪代码
...
|            | 平衡性   | 每次插入最多需要几次旋转就能达到平衡 | 实现起来的难易程度 |
| ---------- | -------- | ------------------------------------ | ------------------ |
| 红黑树     | 大致平衡 | 3                                    | 简单               |
| 平衡二叉树 | 绝对平衡 | 不确定                               | 麻烦               |
...
1、[共享内存和消息队列](http://c.biancheng.net/view/1208.html)
